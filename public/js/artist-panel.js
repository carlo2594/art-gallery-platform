...
(function initArtistPanel() {
  ...
  function alertToast(type, msg) {
    try {
      console.log("[ArtistPanel]", type, msg);
    } catch (_) {}
  ...
  function renderList(listEl, items, status) {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      ...
      li.textContent =
        status === "draft"
          ? "No hay borradores."
          : status === "submitted"
          ? "No hay obras enviadas."
          : "No hay obras rechazadas recientes.";
      ...
      if (status === "draft") {
        ...
        btnEdit.addEventListener("click", () => {
          ...
          window.location.href =
            "/artists/panel/artworks/new?artworkId=" +
            encodeURIComponent(a._id);
        });
    ...
    try {
      if (status === "draft") {
        const badge = qs("#countDrafts");
        if (badge) badge.textContent = String(items.length);
      } else if (status === "submitted") {
        const badge = qs("#countSubmitted");
        if (badge) badge.textContent = String(items.length);
      } else if (status === "rejected") {
        const badge = qs("#countRejected");
        if (badge) badge.textContent = String(items.length);
  ...
  function fetchList(status) {
    ...
    if (window.api) {
      return window.api
        ...
        .catch((err) => {
          console.error("[ArtistPanel] fetchList error (axios)", err);
          return [];
        });
    }
    return fetch(url, { credentials: "same-origin" })
      ...
      .catch((err) => {
        console.error("[ArtistPanel] fetchList error (fetch)", err);
        return [];
      });
  }
  ...
  function getMissingFieldsForSubmit(art) {
    ...
    try {
      if (!art || typeof art !== "object")
        return ["title", "image", "width_cm", "height_cm", "price_cents"];
      if (!art.imageUrl) missing.push("image");
      if (
        !(
          typeof art.width_cm === "number" &&
          isFinite(art.width_cm) &&
          art.width_cm > 0
        )
      )
        missing.push("width_cm");
      if (
        !(
          typeof art.height_cm === "number" &&
          isFinite(art.height_cm) &&
          art.height_cm > 0
        )
      )
      ...
      if (
        !(
          typeof art.price_cents === "number" &&
          isFinite(art.price_cents) &&
          art.price_cents > 0
        )
      )
  ...
  function highlightMissingInForm(missing, art) {
    try {
      ...
      [
        ...
        const el =
          qs(`input[name="${n}"]`, form) || qs(`textarea[name="${n}"]`, form);
        ...
        if (el) {
          ...
          el.addEventListener(
            ...
            function onchg() {
              ...
              el.removeEventListener(ev, onchg);
            },
            { once: true }
          );
        }
    ...
    getReq
      .then((res) => {
        ...
        const req = window.api
          ? window.api.patch(url)
          : fetch(url, { method: "PATCH", credentials: "same-origin" }).then(
              (r) => r.json()
            );
        return req.then(() => {
  ...
  function trashArtwork(id, rowEl) {
    ...
    const req = window.api
      ? window.api.patch(url)
      : fetch(url, { method: "PATCH", credentials: "same-origin" }).then((r) =>
          r.json()
        );
    req
      .then(() => {
  ...
  function loadLists() {
    ...
    Promise.all([
      ...
      fetchList("submitted"),
      fetchList("rejected"),
    ]).then(([drafts, submitted, rejectedAll]) => {
  ...
  function fillFormWith(art) {
    ...
    setVal(
      ...
      art.height_cm != null ? art.height_cm : ""
    );
  ...
  (function hookAddArtworkLimit() {
    try {
      ...
      btn.addEventListener(
        ...
        function (e) {
          e.preventDefault();
          const maxDrafts = parseInt(
            btn.getAttribute("data-max-drafts") || "10",
            10
          );
          ...
          const targetId = over
            ? "artistDraftLimitModal"
            : "artistAddArtworkModal";
          ...
          if (modalEl && window.bootstrap) {
            ...
            if (over)
              alertToast(
                "warning",
                "Has alcanzado el límite de borradores. Envía a aprobación o elimina algunos."
              );
          }
        },
        true
  ...
  function hookCreateForm() {
    ...
    form.addEventListener("submit", function (e) {
        ...
        if (titleEl) {
          ...
          titleEl.addEventListener(
            ...
            function onin() {
              ...
              titleEl.removeEventListener("input", onin);
            },
            { once: true }
          );
          try {
          ...
          if (fileInput) {
            ...
            fileInput.addEventListener(
              ...
              function onch() {
                ...
                fileInput.removeEventListener("change", onch);
              },
              { once: true }
            );
          }
      ...
      const updateUrl = currentEditId
        ? "/api/v1/artworks/" + encodeURIComponent(currentEditId)
        : null;
      const doRequest = () => {
        if (currentEditId) {
          return window.axios
      ...
      doRequest()
        .then((res) => {
          ...
          if (mode === "send" && artId) {
            ...
            const req2 = window.axios
              ? axios.patch(submitUrl, null, { withCredentials: true })
              : fetch(submitUrl, {
                  method: "PATCH",
                  credentials: "same-origin",
                }).then((r) => r.json());
  ...
  function hookArtworkPanelAddLimit() {
    try {
      ...
      btn.addEventListener(
        ...
        function (e) {
          const max = parseInt(
            btn.getAttribute("data-max-artworks") || "10",
            10
          );
          const badgeDrafts = document.getElementById("countDrafts");
          const badgeSubmitted = document.getElementById("countSubmitted");
          const drafts =
            parseInt((badgeDrafts && badgeDrafts.textContent) || "0", 10) || 0;
          const submitted =
            parseInt(
              (badgeSubmitted && badgeSubmitted.textContent) || "0",
              10
            ) || 0;
          ...
          e.preventDefault();
          const detail =
            overDrafts && overSubmitted
              ? "Ya alcanzaste el número máximo de obras en borrador y enviadas."
              : overDrafts
              ? "Ya tienes el máximo de obras en borrador."
              : "Ya tienes el máximo de obras enviadas a revisión.";
          alertToast(
            "warning",
            detail +
              " Espera a que se aprueben o se rechacen algunas obras, o elimina las que no necesites, antes de agregar nuevas."
          );
        },
        true
      );
