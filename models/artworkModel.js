...
const artworkSchema = new mongoose.Schema(
  {
    ...
    review: {
      ...
      rejectReason: {
        ...
        maxlength: [
          100,
          "La razón del rechazo no puede superar 100 caracteres.",
        ],
      },
...
artworkSchema.virtual("aspectRatio").get(function () {
  if (
    typeof this.width_cm === "number" &&
    typeof this.height_cm === "number" &&
    this.height_cm !== 0
  ) {
...
artworkSchema.methods.markSold = function (opts = {}) {
  ...
  this.sale = {
    price_cents:
      typeof opts.price_cents === "number"
        ? Math.max(0, Math.round(opts.price_cents))
        : this.price_cents,
...
artworkSchema.methods.reserve = function (untilDate) {
  ...
  this.reservedUntil =
    untilDate instanceof Date ? untilDate : new Date(untilDate);
  return this.save();
};
...
artworkSchema.statics.findByPriceRange = function ({
  min = 0,
...
artworkSchema.pre("save", async function (next) {
  ...
  if (this.createdAt) {
    ...
    this.createdAt_en = date.toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
...
artworkSchema.pre("save", function (next) {
  try {
    this._shouldResyncExhibitions = !!(
      this.isModified("status") || this.isModified("deletedAt")
    );
  } catch (_) {
...
artworkSchema.post("save", async function (doc, next) {
  ...
  try {
    ...
    const exIds = Array.from(
      new Set((doc.exhibitions || []).map((id) => String(id)))
    );
    ...
    for (const ex of exhibitions) {
      const artIds = Array.from(
        new Set((ex.artworks || []).map((id) => String(id)))
      );
      let artistIds = [];
      if (artIds.length) {
        // Only artists from visible artworks (approved and not trashed)
        artistIds = await ArtworkModel.find({
          _id: { $in: artIds },
          status: "approved",
          deletedAt: null,
        }).distinct("artist");
    ...
    try {
      console.error("artwork post-save resync participants error:", err);
    } catch (_) {}
    return next();
  }
});
